...
interface AnalysisInsightsProps {
  selectedViewType: "impression" | "click";
  onClickValue: string;
...
const AnalysisInsights = ({
  ...
  const detailsPayload = useMemo<DashboardPayload | undefined>(
    () =>
    ...
    switch (viewType) {
      case "table":
        if (!fraudSubCategoryDetailsLoading && tableColumns.length === 0) {
          return <div className="w-full flex items-center h-[30vh]">{noDataFound()}</div>;
        }
      ...
      case "carousel":
        return carouselData.length > 0 ? (
          ...
          <div className="w-full flex items-center h-[30vh]">{noDataFound()}</div>
        );
