...
export default function Publisher({
  ...
  selectedTypeFromDashboard: "impression" | "click";
}) {
  ...
  const {
    ...
    selectedTypeFromDashboard,
    publisherSummaryPayload,
    !!publisherSummaryPayload && !isPackageLoading
  );

  // Event Type List API
  const { data: eventTypeListData, isLoading: isLoadingEventTypeFilter } =
    useEventTypeList(eventTypeListPayload, !!eventTypeListPayload && !isPackageLoading);
  ...
  const { data: installChartData, isLoading: installChartLoading } =
    useClickConversion(
      selectedTypeFromDashboard,
      installChartPayload,
      !!installChartPayload && isDialogOpen
    );
  ...
  }, [eventTypeOptions, selectedEventTypesChart.length]);
  ...
  const handleSelectedRadioButton = useCallback((data: any) => {
  ...
  // Reset page to 1 when type or filters change (but not on initial mount)
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [selectedTypeFromDashboard, selectedRadioButton, debouncedSearchTerm, publisherfilter, campaignfilter, countryfilter, selectedEventTypes]);

  // Listen for the custom event
  useEffect(() => {
    const handleShowDetails = (event: CustomEvent) => {
      setSelectedPublisher(event?.detail?.publisherId || null);
  ...
  return (
    <div className="w-full backdrop-blur-lg bg-background/80 dark:bg-card/80 border border-border/40 rounded-xl shadow-lg p-2 transition-all duration-300">
      ...
      <ResizableTable
        ...
        isLoading={isLoading || isPackageLoading}
