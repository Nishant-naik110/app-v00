...
import type { PublisherApiResponse } from "./useDashboard";
...
export const buildPublisherFilter = (
  ...
  if (data["Whitelisted Publisher"] && data["Whitelisted Publisher"].length > 0) {
    filters.push({
      label: "Whitelisted Publisher",
      checked: isAllSelected || data["Whitelisted Publisher"].some((pub) => selected.includes(pub)),
      subItems: data["Whitelisted Publisher"].map((pub) => ({
        label: pub,
        checked: isAllSelected || selected.includes(pub),
      })),
    });
