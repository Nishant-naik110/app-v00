...
import MailingListPage from "@/app/(app)/integrity/reportingtool/mail/page";
                        ...
                        <div key={index} className="flex gap-2 items-center">
                          <div className="flex-1">
                            <Input
                              ...
                              onChange={formik.handleChange}
                              onBlur={formik.handleBlur}
                              className={`dark:text-white h-10 rounded-md ${emailTouched && emailError ? "border-red-500" : ""}`}
                            />
                            {emailTouched && emailError && <p className="mt-1 text-subBody text-red-600">{emailError}</p>}
                          </div>
                          {!isViewMode && (
                            <div className="flex gap-2 shrink-0">
                              {isLastItem && (                               
                                  ...
                                  className={`h-6 w-6 bg-primary p-1 text-white rounded-full flex items-center justify-center hover:border-primary/30 transition-colors ${hasValue ? "" : "border border-primary"}`} />                             
                              )}
                              {formik.values.emails.length > 1 && (
                               
                                  <Minus
                                  onClick={() => arrayHelpers.remove(index)}
                                   className="h-6 w-6 bg-primary p-1 text-white border border-primary rounded-full flex items-center justify-center hover:border-primary/30 transition-colors"
                                    />
...
const mailingList  = () => {
    ...
    () => [
      {
        ...
        key: "mailing_list_name",
        render: (data: MailingListListItem) => (
          <div className="text-left">{data?.mailing_list_name || "-"}</div>
        ),
      },
      {
        title: "Created By",
      ...
      },
      {
        ...
        key: "created_at",
        render: (data: MailingListListItem) => {
          if (!data?.created_at) return <div className="text-left">-</div>;
          try {
            return <div className="text-left">{format(new Date(data.created_at), "yyyy-MM-dd HH:mm:ss")}</div>;
          } catch {
      ...
      },
      {
        ...
        key: "updated_by",
        render: (data: MailingListListItem) => (
          <div className="text-left">{data?.updated_by || "-"}</div>
        ),
      },
      {
        title: "Updated Date",
        key: "updated_at",
        render: (data: MailingListListItem) => {
          if (!data?.updated_at) return <div className="text-left">-</div>;
          try {
            return <div className="text-left">{format(new Date(data.updated_at), "yyyy-MM-dd HH:mm:ss")}</div>;
          } catch {
            return <div className="text-left">-</div>;
          }
        },
          ...
          return (
            <div className="flex justify-center">
              <Switch
                ...
                checked={isActive}
                onCheckedChange={(checked) => handleStatusChange(data?.id || 0, checked)}
              />
            </div>
          );
        },
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <div className="relative bg-card rounded-md mt-2">
        <div className="p-4">
