...
import { debounce, downloadURI, formatDate } from "@/lib/utils";
...
const ReportingToolPage = () => {
  ...
  const confirmDelete = useCallback(() => {
    if (!rowToDelete || !selectedPackage) return;
  ...
  const handleDownload = useCallback((item: ReportListItem) => {
    if (item?.report_s3_link) {
      downloadURI(item.report_s3_link,`${item.report_name}.csv`);
    }
  }, []);
            ...
            <div className="flex justify-center">
              <Switch
                ...
                onCheckedChange={(checked) =>
                  handleStatusChange(data?.id || 0, checked)
                }
              />
            </div>
          );
